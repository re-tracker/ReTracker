...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="CoTracker3 online runner (writes standardized result.npz).")
...
def main() -> None:
    ...
    T_total = int(video_btchw.shape[1])

    queries = load_queries_txt(args.queries)
    queries = shift_queries_for_clip(queries, start=int(args.start), clip_len=T_total)
    q = torch.from_numpy(queries.txy).unsqueeze(0).to(device)

    ckpt_path = str(Path(args.ckpt).resolve())
    model = build_cotracker(checkpoint=ckpt_path, offline=False, window_len=int(args.window_len), v2=False)
    ...
    try:
        ...
        pred_tracks, pred_vis = _run_one(q)
        ...
        tracks_chunks = torch.empty((1, T_total, N, 2), device="cpu", dtype=torch.float32)
        vis_chunks = torch.empty((1, T_total, N), device="cpu", dtype=torch.float32)
        for s in range(0, N, qbs):
            ...
            torch.cuda.empty_cache()
        pred_tracks, pred_vis = tracks_chunks, vis_chunks

    torch.cuda.synchronize() if device == "cuda" else None
    dt = time.time() - t0
    print(f"[Info] Done. Time: {dt:.3f}s")

    if pred_vis.ndim == 4 and pred_vis.shape[-1] == 1:
        pred_vis = pred_vis[..., 0]
    if pred_vis.dtype != torch.bool:
        pred_vis = pred_vis > float(args.vis_thr)

    save_result_npz(
        out_npz=args.out_npz,
        method="cotracker3_online",
        video_path=args.video,
        resized_hw=resized_hw,
        queries=queries,
        tracks_xy_tn2=pred_tracks[0].detach().cpu().numpy().astype(np.float32, copy=False),
        visibles_tn=pred_vis[0].detach().cpu().numpy().astype(bool, copy=False),
