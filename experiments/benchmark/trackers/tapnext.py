...
class TapNextConfig:
    ckpt: Path
    infer_hw: tuple[int, int] = (256, 256)  # (H, W) fixed for TapNext


class TapNextTracker(ExternalRunnerMixin, Tracker):
    def __init__(self, *, spec: ExternalRunnerSpec, cfg: TapNextConfig, repo_root: Path) -> None:
        self.spec = spec
        self.cfg = cfg
