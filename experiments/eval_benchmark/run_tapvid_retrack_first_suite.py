...
"""
Multi-method TAP-Vid evaluation suite for the custom "retrack" dataset.

Dataset format:
  <dataset_root>/retrack.pkl
  The pickle is a dict mapping seq_name -> {"video": ..., "points": ..., "occluded": ...}
  with point coordinates normalized to [0, 1] (TAP-Vid style).

This script mirrors `run_tapvid_davis_first_suite.py` but points at `retrack.pkl`.
"""
...
# Support running as a script: `python experiments/eval_benchmark/run_tapvid_retrack_first_suite.py ...`
_root = _repo_root()
...
def _make_tracker(method: str, *, cfg: SuiteConfig, repo_root: Path, ld_preload: str | None) -> Any:
    ...
    if method == "trackon2":
        return TrackOn2Tracker(
            spec=ExternalRunnerSpec(
                env_name=cfg.trackon_env, runner_script=runners / "trackon2_runner.py", ld_preload=ld_preload
            ),
            cfg=TrackOn2Config(
                trackon_root=cfg.third_party_root / "track_on",
                ckpt=cfg.trackon_ckpt,
                config=cfg.trackon_config,
                dataset_name="tapvid_retrack_first",
            ),
            repo_root=repo_root,
        )
    ...
    if method == "cotracker3_offline":
        return CoTracker3OfflineTracker(
            ...
            # ReTrack videos are long (hundreds of frames). Reduce CoTracker's internal
            # interpolation resolution to keep memory bounded on crowded GPUs.
            cfg=CoTracker3Config(
                cotracker_root=cfg.third_party_root / "co-tracker",
                ckpt=cfg.cotracker_offline_ckpt,
                interp_hw=(256, 256),
            ),
            repo_root=repo_root,
        )
...
def main() -> None:
    ...
    p = argparse.ArgumentParser(description="Multi-method TAP-Vid ReTrack (first) evaluation suite (subprocess conda envs).")
    p.add_argument("--dataset-root", type=str, required=True, help="Dataset root containing retrack.pkl")
    p.add_argument("--out-dir", type=str, default=str(repo_root / "outputs/eval_benchmark"), help="Output directory")

    # Default excludes retracker (user request).
    p.add_argument("--methods", type=str, default="trackon2,cotracker3_offline,cotracker3_online,tapnext", help="Comma-separated methods to run")
    ...
    pkl_path = resolve_existing_pickle_path(dataset_root / "retrack.pkl")
    if not pkl_path.exists():
        raise FileNotFoundError(f"ReTrack pickle not found: {pkl_path}")

    out_root = Path(args.out_dir).expanduser().resolve() / "tapvid_retrack_first"
    ...
    suite_cfg = SuiteConfig(
        ...
        trackon_ckpt=Path(args.trackon_ckpt).expanduser().resolve()
        if args.trackon_ckpt
        else (ckpt_dir / "trackon2_dinov2_checkpoint.pt").resolve(),
    ...
    compare: dict[str, Any] = {"methods": {}, "dataset": "tapvid_retrack_first"}
    ...
    for method in methods:
        ...
        result_path = out_dir_method / "result_eval_tapvid_retrack_first.json"
